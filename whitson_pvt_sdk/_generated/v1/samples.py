...
def create_sample_deprecated(transport: HTTPTransport, data: CreateSampleModel) -> GetSampleModel:
    body = transport.post("/samples/create", body=data.model_dump(exclude_unset=True))
    return GetSampleModel.model_validate(body)


def create_samples_deprecated(
    transport: HTTPTransport, data: CreateSampleListModel
) -> GetSampleListModel:
    body = transport.post(
        "/samples/create-many", body=[model.model_dump(exclude_unset=True) for model in data.root]
    )
    return GetSampleListModel.model_validate(body)
