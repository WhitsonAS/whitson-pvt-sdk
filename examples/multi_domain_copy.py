...
from whitson_pvt_sdk.v2.models import CreateRegionModel
...
def _build_region_map(
    ...
    for spec in config.create_regions:
        if spec.name not in region_map:
            created = target.regions.create(CreateRegionModel(**spec.model_dump(exclude_none=True)))
            if created.name:
...
def _copy_one_report(
    ...
    source = WhitsonPVTClient(
        ...
        auth0_domain=domain_auth.auth0_domain,
        audience=domain_auth.audience,
...
def _run(config: MultiDomainCopyConfig) -> int:
    target = WhitsonPVTClient(
        ...
        auth0_domain=config.target.auth0_domain,
        audience=config.target.audience,
